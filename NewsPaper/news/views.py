...
class PostDetailView(DetailView):
    ...
    queryset = Post.objects.all()#Post.category_id()
...
def add_subscribe(request, **kwargs):
    ...
    print('Пользователь', request.user, 'добавлен в подписчики категории:', Post.objects.get(pk=pk))
    Post.objects.get(pk=pk).subscribers.add(request.user)
...
def del_subscribe(request, **kwargs):
    pk = request.GET.get('pk', )
    print('Пользователь', request.user, 'удален из подписчиков категории:', Post.objects.get(pk=pk))
    Post.objects.get(pk=pk).subscribers.remove(request.user)
    return redirect('/news/')
